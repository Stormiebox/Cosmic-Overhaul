...
function TransferCrewGoods.createCrewTab(tabbedWindow)
    ...
    local label = crewTab:createLabel(Rect(lhSplit.bottom.topLeft, rhSplit.bottom.bottomRight),
        "[Drag & Drop / Shift Click] Transfer Passengers"%_t, 12)
...
function TransferCrewGoods.createCargoTab(tabbedWindow)
    ...
    TransferCrewGoods.playerCargoSearch.backgroundText = "Search Cargo..."%_t
    TransferCrewGoods.playerCargoSearch.clearOnClick = true

    TransferCrewGoods.selfCargoSearch = cargoTab:createTextBox(rightHSplit.top, "refreshUI")
    ...
    local leftFrame = cargoTab:createScrollFrame(leftHSplit.bottom)
    local rightFrame = cargoTab:createScrollFrame(rightHSplit.bottom)

    playerTransferAllCargoButton = leftFrame:createButton(Rect(), "/* Goods */Transfer All >>"%_t,
        "onPlayerTransferAllCargoPressed")
    leftLister:placeElementCenter(playerTransferAllCargoButton)

    selfTransferAllCargoButton = rightFrame:createButton(Rect(), "/* Goods */<< Transfer All"%_t,
        "onSelfTransferAllCargoPressed")
...
function TransferCrewGoods.createFightersTab(tabbedWindow)
    ...
    playerTransferAllFightersButton = fightersTab:createButton(Rect(), "Transfer All >>"%_t,
        "onPlayerTransferAllFightersPressed")
    leftLister:placeElementCenter(playerTransferAllFightersButton)

    selfTransferAllFightersButton = fightersTab:createButton(Rect(), "<< Transfer All"%_t,
        "onSelfTransferAllFightersPressed")
...
function TransferCrewGoods.createTorpedoesTab(tabbedWindow)
    local torpedoesTab = tabbedWindow:createTab("Torpedoes"%_t, "data/textures/icons/missile-pod.png",
        "Exchange Torpedoes"%_t)
...
function TransferCrewGoods.refreshCrewUI(playerShip, ship)
    ...
    for _, p in pairs(TransferCrewGoods.getSortedCrewmen(playerShip)) do
        ...
        if not crewman.specialist then
            ...
            caption = "${profession} (Specialist level ${level})"%_t%
            { profession = crewman.profession:name(num), level = crewman.level }
        end
...
function TransferCrewGoods.refreshCargoUI(playerShip, ship)
    ...
    -- restore textbox values
    -- Cosmic Overhaul: Custom index mapping to safely handle filtered/sorted arrays
    TransferCrewGoods.playerButtonToOriginalIndex = {}
    TransferCrewGoods.selfButtonToOriginalIndex = {}
    TransferCrewGoods.playerTextBoxToOriginalIndex = {}
    TransferCrewGoods.selfTextBoxToOriginalIndex = {}

    -- Cosmic Overhaul: Alphabetical sorting and live-filtering logic for massive cargo lists
    local function getSortedAndFilteredCargos(entity, filterText)
        local result = {}
        if not entity then return result end
        filterText = filterText and filterText:lower() or ""
        for i = 0, entity.numCargos-1 do
            local good, amount = entity:getCargo(i)
            if good and amount > 0 then
                -- Cosmic Overhaul: Added plain text flag (1, true) to string.find to prevent malformed pattern crashes
                if filterText == "" or good.name:lower():find(filterText, 1, true) or good:displayName(amount):lower():find(filterText, 1, true) then
                    table.insert(result, { good = good, amount = amount, originalIndex = i })
                end
            end
        end
        table.sort(result, function(a, b) return a.good:displayName(a.amount) < b.good:displayName(b.amount) end)
        return result
    end

    local playerCargos = getSortedAndFilteredCargos(playerShip, TransferCrewGoods.playerCargoSearch.text)
    local selfCargos = getSortedAndFilteredCargos(ship, TransferCrewGoods.selfCargoSearch.text)
    ...
    for i, _ in pairs(playerCargoBars) do
        ...
        local cargoData = playerCargos[i]
        ...
        if not cargoData then
            ...
            local good = cargoData.good
            local amount = cargoData.amount
            TransferCrewGoods.playerButtonToOriginalIndex[button.index] = cargoData.originalIndex
            TransferCrewGoods.playerTextBoxToOriginalIndex[box.index] = cargoData.originalIndex
        ...
        if not cargoData then
            ...
            local good = cargoData.good
            local amount = cargoData.amount
            TransferCrewGoods.selfButtonToOriginalIndex[button.index] = cargoData.originalIndex
            TransferCrewGoods.selfTextBoxToOriginalIndex[box.index] = cargoData.originalIndex
...
function TransferCrewGoods.onPlayerTransferAllCrewPressed(button)
    -- Cosmic Overhaul: Check if Right Mouse Button is held down for "Stacking" (Transfer only what's already on the other side)
    local stack = Mouse():mouseDown(3)
    invokeServerFunction("transferAllCargo", Player().craftIndex, false, stack)
end

function TransferCrewGoods.onSelfTransferAllCrewPressed(button)
    -- Cosmic Overhaul: Check if Right Mouse Button is held down for "Stacking" (Transfer only what's already on the other side)
    local stack = Mouse():mouseDown(3)
    invokeServerFunction("transferAllCargo", Player().craftIndex, true, stack)
end
...
function TransferCrewGoods.onPlayerTransferCargoTextEntered(textBox)
    ...
    -- get available amount
    -- Cosmic Overhaul: Use our safely mapped original index instead of the raw UI index
    local cargoIndex = TransferCrewGoods.playerTextBoxToOriginalIndex[textBox.index]
...
function TransferCrewGoods.onSelfTransferCargoTextEntered(textBox)
    ...
    -- get available amount
    -- Cosmic Overhaul: Use our safely mapped original index instead of the raw UI index
    local cargoIndex = TransferCrewGoods.selfTextBoxToOriginalIndex[textBox.index]
    if not cargoIndex then return end
...
function TransferCrewGoods.onPlayerTransferCargoPressed(button)
    ...
    -- check which cargo
    -- Cosmic Overhaul: Safely retrieve the mapped original index to prevent sending the wrong item!
    local cargoIndex = TransferCrewGoods.playerButtonToOriginalIndex[button.index]
...
function TransferCrewGoods.onSelfTransferCargoPressed(button)
    ...
    -- check which cargo
    -- Cosmic Overhaul: Safely retrieve the mapped original index to prevent sending the wrong item!
    local cargoIndex = TransferCrewGoods.selfButtonToOriginalIndex[button.index]
    if cargoIndex == nil then return end
...
function TransferCrewGoods.transferCargo(cargoIndex, otherIndex, selfToOther, amount)
    ...
    if receiver.freeCargoSpace < good.size*amount then
        -- Cosmic Overhaul: Instead of completely failing, transfer as much as possible to the receiving ship
...
function TransferCrewGoods.transferAllCargo(otherIndex, selfToOther, stack)
    local sender
    local receiver
    ...
    for good, amount in pairs(cargos) do
        -- Cosmic Overhaul: If stacking is enabled (RMB held), only transfer cargo that already exists in the receiver's inventory
        if stack then
            local _, receiverAmount = receiver:getCargo(good)
            if not receiverAmount or receiverAmount <= 0 then
                goto continue
            end
        end
        ...
        if not targetLauncher:addTorpedo(torpedo, shaftIndex) then
            ...
            if torpedoesChanged == false then
                if shaftIndex == -1 then
                    ...
                    player:sendChatMessage("", ChatMessageType.Error, "Not enough free space in torpedo shaft %1%."%_t,
                        shaftIndex+1)
                end
            end
