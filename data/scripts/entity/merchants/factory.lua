...
-- Data persistence to ensure shuttle upgrades and analytics survive server restarts/sector unloads
local old_secure = Factory.secure
function Factory.secure()
	local data = old_secure() or {}
	data.co_shuttleVolume = Factory.shuttleVolume
	data.co_runtime = runtime
	data.co_productionStateRegister = productionStateRegister
	return data
end

local old_restore = Factory.restore
function Factory.restore(data)
	old_restore(data)
	if data then
		if data.co_shuttleVolume then Factory.shuttleVolume = data.co_shuttleVolume end
		if data.co_runtime then runtime = data.co_runtime end
		if data.co_productionStateRegister then productionStateRegister = data.co_productionStateRegister end
	end
end
...
local DATA_REFRESH_FREQUENCY = 30 -- controls how often factories call in with new data, in seconds.
...
function updateGalaxy(allianceFactory)
	...
	factoryData['title'] = self.translatedTitle or self.name or ""
...
local refreshTime = DATA_REFRESH_FREQUENCY -- setting it to this so the first update is instantaneous
...
function Factory.updateProduction(timeStep)
	...
	if Owner().isPlayer or Owner().isAlliance then
		...
		updateProductionStateRegister(timeStep, Factory.productionError) -- hook into vanilla factory error state

		if refreshTime > DATA_REFRESH_FREQUENCY then
...
function updateProductionStateRegister(timeStep, currentError)
	local reason = "Running"%_T
	if currentError == nil or currentError == "" then
		reason = "Running"%_T
	else
		reason = currentError
	end
