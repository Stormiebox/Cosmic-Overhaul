...
function SectorTurretGenerator:getTurretSeed(x, y, weaponType, rarity)
    -- Exotic and Legendary weapons have a 50% chance to be completely wild/unique
    if rarity.type >= RarityType.Exotic and self.random:test(0.5) then
        return self.random:createSeed(), x, y
    end

    -- Introduce a micro-variance (1 to 3) so the exact same sector/rarity/weaponType combination
    ...
    local seedString = string.format("%s_%s_%s_%s_%s_%s",
        ...
        tostring(weaponType), tostring(rarity.type), tostring(microVariance))
