...
function ShipUtility.addDisruptorEquipment(craft)
    ...
    craft:setTitle("${toughness}Disruptor ${class}"%_T,
        { toughness = "", class = ShipUtility.getMilitaryNameByVolume(craft.volume) })
...
function ShipUtility.addCIWSEquipment(craft)
    ...
    craft:setTitle("${toughness}CIWS ${class}"%_T,
        { toughness = "", class = ShipUtility.getMilitaryNameByVolume(craft.volume) })
    craft:setValue("is_armed", true)
...
function ShipUtility.addTorpedoBoatEquipment(craft)
    ...
    craft:setTitle("${toughness}Torpedo-${class}"%_T,
        { toughness = "", class = ShipUtility.getMilitaryNameByVolume(craft.volume) })
    craft:setValue("is_armed", true)
...
function ShipUtility.addArtilleryEquipment(craft)
    ...
    craft:setTitle("${toughness}Artillery ${class}"%_T,
        { toughness = "", class = ShipUtility.getMilitaryNameByVolume(craft.volume) })
    craft:setValue("is_armed", true)
...
function ShipUtility.addPersecutorEquipment(craft)
    ...
    craft:setTitle("${toughness}Persecutor ${class}"%_T,
        { toughness = "", class = ShipUtility.getMilitaryNameByVolume(craft.volume) })
    craft:setValue("is_armed", true)
...
function ShipUtility.addBlockerEquipment(craft)
    ...
    craft:setTitle("${toughness}Hyperspace Blocker ${class}"%_T,
        { toughness = "", class = ShipUtility.getMilitaryNameByVolume(craft.volume) })
    craft:setValue("is_armed", true)
...
function ShipUtility.addCarrierEquipment(craft, fighters)
    ...
    craft:setTitle("${toughness}Carrier ${class}"%_T,
        { toughness = "", class = ShipUtility.getMilitaryNameByVolume(craft.volume) })
    craft:setValue("is_armed", true)
...
-- Vanilla code Above this line
-- Cosmic Overhaul: Cached tables for massive performance gains during sector generation
-- 12 volume thresholds results in 13 distinct size tiers (Index 1 to 13)
...
-- Accessor functions for external scripts that might request the cached volumes
function ShipUtility.getMilitaryMaxVolumes()
    return militaryMaxVolumes
end

function ShipUtility.getCivilMaxVolumes()
    return civilMaxVolumes
end

function ShipUtility.getMilitaryShipSizeIndex(volume, maxindex)
    -- Optimized O(n) linear search. Returns immediately upon finding the first
    -- threshold larger than the ship's volume, safely capping at maxindex.
    for i = 1, #militaryMaxVolumes do
        if volume < militaryMaxVolumes[i] then
...
function ShipUtility.getCivilShipSizeIndex(volume, maxindex)
    -- Optimized O(n) linear search. Returns immediately upon finding the first
    -- threshold larger than the ship's volume, safely capping at maxindex.
    for i = 1, #civilMaxVolumes do
        if volume < civilMaxVolumes[i] then
            return math.min(i, maxindex)
        end
    end
...
function ShipUtility.getMinerNameByVolume(volume)
    -- Cosmic Overhaul: Industrial-themed naming conventions for mining vessels
    -- Array size: 13 names (Matches the 12 volume thresholds + 1 overflow)
    local names =
    {
        {name = "Light Prospector /* ship title */"%_T, type = MinerShipTypes.LightMiner},
        {name = "Prospector /* ship title */"%_T, type = MinerShipTypes.LightMiner},
        {name = "Heavy Prospector /* ship title */"%_T, type = MinerShipTypes.Miner},
        {name = "Light Miner /* ship title */"%_T, type = MinerShipTypes.Miner},
        {name = "Medium Miner /* ship title */"%_T, type = MinerShipTypes.Miner},
        {name = "Heavy Miner /* ship title */"%_T, type = MinerShipTypes.HeavyMiner},
        {name = "Mining Barge /* ship title */"%_T, type = MinerShipTypes.HeavyMiner},
        {name = "Heavy Mining Barge /* ship title */"%_T, type = MinerShipTypes.MiningMoloch},
        {name = "Extraction Vessel /* ship title */"%_T, type = MinerShipTypes.MiningMoloch},
        {name = "Heavy Extraction Vessel /* ship title */"%_T, type = MinerShipTypes.MiningMoloch},
        {name = "Asteroid Crusher /* ship title */"%_T, type = MinerShipTypes.MiningMoloch},
        {name = "Planet Cracker /* ship title */"%_T, type = MinerShipTypes.MiningMoloch},
        {name = "Mining Moloch /* ship title */"%_T, type = MinerShipTypes.MiningMoloch},
    }
...
function ShipUtility.getFreighterNameByVolume(volume)
    -- Cosmic Overhaul: Logistics-themed naming conventions for freighters
    -- Array size: 13 names
    local names =
    {
        {name = "Cargo Shuttle /* ship title */"%_T, type = FreighterShipTypes.Loader},
        {name = "Light Loader /* ship title */"%_T, type = FreighterShipTypes.Loader},
        {name = "Heavy Loader /* ship title */"%_T, type = FreighterShipTypes.Transporter},
        {name = "Light Transporter /* ship title */"%_T, type = FreighterShipTypes.Transporter},
        {name = "Transporter /* ship title */"%_T, type = FreighterShipTypes.Lifter},
        {name = "Heavy Transporter /* ship title */"%_T, type = FreighterShipTypes.Lifter},
        {name = "Light Freighter /* ship title */"%_T, type = FreighterShipTypes.CargoTransport},
        {name = "Freighter /* ship title */"%_T, type = FreighterShipTypes.Freighter},
        {name = "Heavy Freighter /* ship title */"%_T, type = FreighterShipTypes.Freighter},
        {name = "Cargo Hauler /* ship title */"%_T, type = FreighterShipTypes.CargoHauler},
        {name = "Heavy Cargo Hauler /* ship title */"%_T, type = FreighterShipTypes.CargoHauler},
        {name = "Superfreighter /* ship title */"%_T, type = FreighterShipTypes.HeavyCargoHauler},
        {name = "Logistics Leviathan /* ship title */"%_T, type = FreighterShipTypes.HeavyCargoHauler},
    }
    local size = ShipUtility.getCivilShipSizeIndex(volume, #names)
    return names[size].name, names[size].type
end
...
function ShipUtility.getTraderNameByVolume(volume)
    -- Cosmic Overhaul: Commerce-themed naming conventions for trading vessels
    -- Array size: 13 names
    local names =
    {
        {name = "Light Courier /* ship title */"%_T, type = TraderShipTypes.Trader},
        {name = "Courier /* ship title */"%_T, type = TraderShipTypes.Trader},
        {name = "Heavy Courier /* ship title */"%_T, type = TraderShipTypes.Trader},
        {name = "Trade Vessel /* ship title */"%_T, type = TraderShipTypes.Trader},
        {name = "Merchant Vessel /* ship title */"%_T, type = TraderShipTypes.Merchant},
        {name = "Heavy Merchant Vessel /* ship title */"%_T, type = TraderShipTypes.Merchant},
        {name = "Commerce Ship /* ship title */"%_T, type = TraderShipTypes.Merchant},
        {name = "Heavy Commerce Ship /* ship title */"%_T, type = TraderShipTypes.Merchant},
        {name = "Trade Galleon /* ship title */"%_T, type = TraderShipTypes.Salesman},
        {name = "Exchange Hub /* ship title */"%_T, type = TraderShipTypes.Salesman},
        {name = "Grand Exchange /* ship title */"%_T, type = TraderShipTypes.Salesman},
        {name = "Commercial Colossus /* ship title */"%_T, type = TraderShipTypes.Salesman},
        {name = "Trade Leviathan /* ship title */"%_T, type = TraderShipTypes.Salesman},
    }
    local size = ShipUtility.getCivilShipSizeIndex(volume, #names)
    return names[size].name, names[size].type
end
...
function ShipUtility.getMilitaryNameByVolume(volume)
    -- Cosmic Overhaul: Sci-Fi naval terminology for military vessels
    -- Array size: 13 names
    local names =
    {
        {name = "Interceptor /* ship title */"%_T, type = MilitaryShipTypes.Scout},
        {name = "Scout /* ship title */"%_T, type = MilitaryShipTypes.Sentinel},
        {name = "Corvette /* ship title */"%_T, type = MilitaryShipTypes.Corvette},
        {name = "Frigate /* ship title */"%_T, type = MilitaryShipTypes.Frigate},
        {name = "Destroyer /* ship title */"%_T, type = MilitaryShipTypes.Destroyer},
        {name = "Cruiser /* ship title */"%_T, type = MilitaryShipTypes.Cruiser},
        {name = "Heavy Cruiser /* ship title */"%_T, type = MilitaryShipTypes.Cruiser},
        {name = "Battlecruiser /* ship title */"%_T, type = MilitaryShipTypes.Battleship},
        {name = "Battleship /* ship title */"%_T, type = MilitaryShipTypes.Battleship},
        {name = "Dreadnought /* ship title */"%_T, type = MilitaryShipTypes.Dreadnought},
        {name = "Juggernaut /* ship title */"%_T, type = MilitaryShipTypes.Dreadnought},
        {name = "Titan /* ship title */"%_T, type = MilitaryShipTypes.Dreadnought},
        {name = "Leviathan /* ship title */"%_T, type = MilitaryShipTypes.Dreadnought},
    }
