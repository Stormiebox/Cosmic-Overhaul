...
function initialize(goodName, amount, giverIndex, reward)
    ...
    if onClient() then
        ...
        local giverFactionIndex = giver and giver.factionIndex
            ...
            if giverInsideBarrier == Balancing_InsideRing(coord.x, coord.y) then
                if regular or home then
                    ...
                    if destinations[specs.generationTemplate.path] then
                        local atWar = false
                        if specs.factionIndex and giverFactionIndex then
                            if Galaxy():getFactionRelationStatus(giverFactionIndex, specs.factionIndex) == RelationStatus.War then
                                atWar = true
                            end
                        end
